...
<div class="main-card mb-3 card">
    <div class="card-body">
        <form action="{{ route('empresas.update',$company->id) }}"  method="POST" id="editar-empresa" class="col-md-10 mx-auto" enctype='multipart/form-data'>
            @csrf
            @method('PUT')
            @if ($errors->any())
                    ...
                    <div class="position-relative form-group">
                        ...
                        <div class="mb-3">
                            <input required type="text" class="form-control" id="name" name="name"
                                placeholder="Introduzca el nombre de la empresa" value="{{ $company->name }}">
                        </div>
                    </div>
                </div>
                <div class="col-md-6 order-1 order-md-2 text-center">
                    <img id="imgPreview" src="{{asset("img/companies/".$company->photo)}}" style="max-height:120px; max-width: 120px; border-style : solid; border-width:1px; border-radius: 16px; padding: 20px;"> <br>
            ...
            <div class="form-group row justify-content-center">
                <button type="submit" class="btn btn-success" name="crear"><i class="fa-regular fa-pen-to-square"></i> ACTUALIZAR</button>
            </div>
        </form>
    </div>
</div>
